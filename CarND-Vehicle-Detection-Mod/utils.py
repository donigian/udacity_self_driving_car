...
def find_cars_lesson(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
...
def draw_labeled_bboxes_lesson(img, labels):
...
def single_img_features_nonviz(img, color_space='RGB', spatial_size=(32, 32),
...
def search_windows(img, windows, clf, scaler, color_space='RGB', 
    ...
    for window in windows:
        ...
        features = single_img_features_nonviz(test_img, color_space=color_space, 
...
def detect_windows_in_heatmap(image):
    hot_windows = []
    threshold = 0
    image[image <= threshold] = 0
    labels = ndi.label(image)
    for cn in range(1, labels[1]+1):
        nonzero = (labels[0] == cn).nonzero()
        nonzeroy = np.array(nonzero[0])
        nonzerox = np.array(nonzero[1])
...
def merge_boxes(windows, image_shape):
    hot_windows = []
    image = None
    if len(windows)>0:
        image = create_heatmap(windows, image_shape)
        hot_windows = detect_windows_in_heatmap(image)
    return hot_windows    

def create_windows(overlap):
    ...
    windows = []
    for a, b in grid:
        windows += slide_window(image, x_start_stop=[None, None], y_start_stop=b,
                        xy_window=a, xy_overlap=(overlap, overlap))
    return windows    
